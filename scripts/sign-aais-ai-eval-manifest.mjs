...
export const aaisAiEvalManifestSigningReceiptContractVersion =
  "aais-ai-eval-manifest-signing-receipt-v2";
...
const maximumEvidenceWindowMs = 366 * 24 * 60 * 60 * 1_000;
const maximumManifestBytes = 1024 * 1024;
const sha256Pattern = /^[a-f0-9]{64}$/;
const safeIdentifierPattern = /^[A-Za-z0-9][A-Za-z0-9._:-]{2,127}$/;
const snapshotModelPattern = /^qwen\d+(?:\.\d+)*-[a-z0-9.-]+-\d{4}-\d{2}-\d{2}$/;
...
export function signAaisAiEvalManifest(input) {
  const expectedModel = readSnapshotModel(input.expectedModel);
  const expectedSnapshotSha256 = readSha256(
    input.expectedSnapshotSha256,
    "AAIS_AI_MANIFEST_EXPECTED_SNAPSHOT_INVALID",
  );
...
function validateUnsignedManifest(input) {
  ...
  validateReleaseEvidence(manifest.releaseEvidence, {
...
function validateReleaseEvidence(evidence, expected) {
  ...
  if (!isRecord(evidence)
    ...
    || runtime.maxTokens !== 600
    || runtime.maxRetries !== 0
    || runtime.requestedSnapshotModel !== expected.expectedModel
    || runtime.observationKind !== "exact-provider-model-id"
    || runtime.observedSnapshotSha256 !== expected.expectedSnapshotSha256
...
function readSnapshotModel(value) {
  const normalized = String(value ?? "").trim();
  if (!snapshotModelPattern.test(normalized)) {
    fail("AAIS_AI_MANIFEST_EXPECTED_MODEL_INVALID");
  }
  return normalized;
}
...
function main() {
  try {
    const result = writeSignedAaisAiEvalManifest({
      ...
      expectedSnapshotSha256:
        process.env.AAIS_AI_MANIFEST_EXPECTED_SNAPSHOT_SHA256,
      signingKeyId: process.env.AAIS_AI_MANIFEST_SIGNING_KEY_ID,
      signingKeyPkcs8: readSigningKeyEnvironment(),
    });
    console.log(JSON.stringify({
      status: "signed",
      contractVersion: aaisAiEvalManifestSigningReceiptContractVersion,
      provider: result.manifest.providerFamily,
...
function readSigningKeyEnvironment() {
  const inline = process.env.AAIS_AI_MANIFEST_SIGNING_KEY_PKCS8?.trim();
  if (inline) return inline;
  const keyPath = process.env.AAIS_AI_MANIFEST_SIGNING_KEY_PKCS8_FILE?.trim();
  if (!keyPath || keyPath.includes("\0")) {
    fail("AAIS_AI_MANIFEST_SIGNING_KEY_INVALID");
  }
  try {

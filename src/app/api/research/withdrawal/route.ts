...
const aaisResearchWithdrawalBodyMaxBytes = 1024;
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const body = await readAaisBoundedJson(request, {
      maxBytes: aaisResearchWithdrawalBodyMaxBytes,
    });
    ...
    if (error instanceof AaisRequestBodyError) {
      return createAaisResearchErrorResponse({
        error,
        route: "/api/research/withdrawal",
        operation: "withdrawal.begin",
        authMode,
        responseInput: {
          ...
          message: error.reason === "too_large"
            ? "AAIS research withdrawal request is too large."
            : "AAIS research withdrawal request is invalid.",
          status: error.status,

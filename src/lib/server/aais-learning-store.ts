...
export function getAaisDatabaseConfiguration(): AaisDatabaseConfiguration | null {
  ...
  if (process.env.NODE_ENV === "production" || process.env.VERCEL_ENV === "production") {
    assertCanonicalProductionDatabaseBinding(candidates);
    const url = process.env.AAIS_DATABASE_URL?.trim();
    return url ? { url, sourceEnv: "AAIS_DATABASE_URL" } : null;
  }
...
function assertCanonicalProductionDatabaseBinding(candidates: AaisDatabaseSourceEnv[]) {
  const forbiddenNames = [
    ...candidates.filter((sourceEnv) => sourceEnv !== "AAIS_DATABASE_URL"),
    "PGHOST",
    "PGHOST_UNPOOLED",
    "PGUSER",
    "PGDATABASE",
  ...
  if (forbiddenNames.length > 0) {
    throw new Error(
      `AAIS production database configuration must expose only AAIS_DATABASE_URL (${forbiddenNames.join(", ")}).`,
    );
  }
}

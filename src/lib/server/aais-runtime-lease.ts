...
export async function acquireAaisRuntimeLease(
  ...
  const configuredProvider = getAaisReleaseMetadata(env).deployment.provider;
  const provider = env.VERCEL === "1" || Boolean(env.VERCEL_ENV)
    ? "vercel"
    : configuredProvider;
  const primaryHeartbeatKey = `${leaseKey}:aliyun-primary`;
  ...
  try {
    let primaryHeartbeat: AaisRuntimeLease["primaryHeartbeat"] = null;
    if (provider === "aliyun") {
      primaryHeartbeat = await renewAaisAliyunPrimaryHeartbeat(
        database,
        primaryHeartbeatKey,
        getPrimaryHeartbeatHolderId(env),
      );
    }
    const result = await database.query(
      `insert into public.aais_runtime_leases (
       ...
       where $4::text <> 'vercel'
          or not exists (
            select 1
              from public.aais_runtime_leases primary_heartbeat
             where primary_heartbeat.lease_key = $5
               and primary_heartbeat.expires_at > clock_timestamp()
          )
...
async function renewAaisAliyunPrimaryHeartbeat(
  database: AaisDatabaseClient,
  heartbeatKey: string,
  holderId: string,
): Promise<NonNullable<AaisRuntimeLease["primaryHeartbeat"]>> {
  const result = await database.query(
    `insert into public.aais_runtime_leases (
       lease_key,
       holder_id,
     ...
     on conflict (lease_key) do update set
       ...
       updated_at = excluded.updated_at
     returning holder_id, generation`,
    [heartbeatKey, holderId, runtimeLeaseTtlSeconds],
  );
  const row = result.rows[0];
  const generation = Number(row?.generation);
  if (
...
export async function releaseAaisPrimaryHeartbeat(
  lease: AaisRuntimeLease,
  input: {
    database?: AaisDatabaseClient;
    env?: Record<string, string | undefined>;
  ...
  const heartbeat = lease.primaryHeartbeat;
  if (!lease.required || !heartbeat) {
    return;
  }
  const env = input.env ?? process.env;
  const database = input.database ?? getRuntimeLeaseDatabase(env);
    ...
    await database.query(
      `delete from public.aais_runtime_leases
        where lease_key = $1
          and holder_id = $2
          and generation = $3`,
      [heartbeat.leaseKey, heartbeat.holderId, heartbeat.generation],
    );
  } catch {
    // Fencing prevents an old failed invocation from deleting a newer heartbeat.
  }
}
...
function getRuntimeLeaseDatabase(
  ...
  if (shouldUseNeonServerlessDriver(configuration.url, env)) {
    return createAaisNeonQueryClient(configuration.url, env);
  }
...
function getPrimaryHeartbeatHolderId(env: Record<string, string | undefined>) {
  const explicit = env.AAIS_WORKER_INSTANCE_ID?.trim();
  if (explicit) {
    if (!/^[A-Za-z0-9][A-Za-z0-9._:-]{2,79}$/.test(explicit)) {
      throw new AaisRuntimeLeaseUnavailableError();
  ...
  const release = getAaisReleaseMetadata(env);
  const prefix = `aliyun:${release.id ?? "development"}`.slice(0, 116);

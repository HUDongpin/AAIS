...
export type AaisReleaseMetadata = {
  id: string | null;
  source: "AAIS_RELEASE_ID" | "VERCEL_GIT_COMMIT_SHA" | "missing";
  deployment: {
...
export function getAaisReleaseMetadata(
  ...
  const provider = readAaisDeploymentProvider(env);
  const vercelGitCommitSha = readSafeFullGitCommitSha(env.VERCEL_GIT_COMMIT_SHA);
  const explicitGitCommitSha = readSafeFullGitCommitSha(
    env.AAIS_DEPLOYMENT_GIT_COMMIT_SHA,
  );
  const configuredReleaseId = readSafeReleaseId(env.AAIS_RELEASE_ID);
  const releaseId = provider === "vercel" && vercelGitCommitSha
    ? vercelGitCommitSha
    : provider === "aliyun" && explicitGitCommitSha === configuredReleaseId
      ? configuredReleaseId
      : provider === "aliyun"
        ? null
        : configuredReleaseId;
  const releaseSource = provider === "vercel" && vercelGitCommitSha
    ? "VERCEL_GIT_COMMIT_SHA" as const
    : releaseId
      ? "AAIS_RELEASE_ID" as const
      : "missing" as const;
...
function readSafeFullGitCommitSha(value: string | undefined) {

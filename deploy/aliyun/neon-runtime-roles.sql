...
-- Provider-neutral PostgreSQL roles for the shared Neon authority. Run only
-- after migrations 0028/0029 and the target identity have been verified.
-- Passwords are deliberately absent; the Owner sets each LOGIN password in a
-- hidden provider or psql prompt and stores it in the matching workload.
do $roles$
begin
  ...
  if not exists (select 1 from pg_roles where rolname = 'aais_app_vercel') then
    create role aais_app_vercel login nosuperuser nocreatedb nocreaterole noinherit noreplication nobypassrls
      connection limit 20;
  end if;
...
-- Reassert the security boundary if a role already existed.
alter role aais_app_aliyun login nosuperuser nocreatedb nocreaterole noinherit noreplication nobypassrls
  connection limit 10;
alter role aais_app_vercel login nosuperuser nocreatedb nocreaterole noinherit noreplication nobypassrls
  connection limit 20;
...
grant usage on schema public to aais_app_aliyun, aais_app_vercel;
...
begin
  ...
  loop
    execute format(
      'grant execute on function %s to aais_app_aliyun, aais_app_vercel',
      runtime_function
    );
  end loop;
end
...
-- Future migrations stay fail-closed until this explicit runtime allowlist is
-- reviewed. The migrator can create objects only while its window is open.

...
describe("AAIS backend learning store", () => {
  ...
  it("fails closed when production exposes any database URL alias", () => {
    vi.stubEnv("NODE_ENV", "production");
    ...
    expect(() => getAaisDatabaseConfiguration()).toThrow(
      "AAIS production database configuration must expose only AAIS_DATABASE_URL (DATABASE_URL).",
    );
    try {
      getAaisDatabaseConfiguration();
    } catch (error) {
  ...
  it("rejects duplicate production aliases even when they resolve to the same URL", () => {
    vi.stubEnv("NODE_ENV", "production");
  ...
  it("rejects raw production PG variables without reflecting their values", () => {
    vi.stubEnv("NODE_ENV", "production");
    vi.stubEnv("AAIS_DATABASE_URL", "postgres://aais:primary-secret@example.test/aais");
    vi.stubEnv("PGHOST", "database-secret.example.test");

    expect(() => getAaisDatabaseConfiguration()).toThrow(
      "AAIS production database configuration must expose only AAIS_DATABASE_URL (PGHOST).",
    );
    try {
      getAaisDatabaseConfiguration();
    } catch (error) {
      expect(String(error)).not.toContain("primary-secret");
      expect(String(error)).not.toContain("database-secret.example.test");
    }
  });

...
| T-03 Private GitHub repo, protected main, CI | Partially implemented; provider plan action pending | Private repo `https://github.com/HUDongpin/AAIS` exists and remote `main` contains the advisory implementation; `.github/workflows/ci.yml`, `.github/workflows/preview-e2e.yml`, Dependabot, repo hygiene checks, dependency-audit guardrails, and the Vercel production build guard in `scripts/guard-vercel-production-deploy.mjs` are present. GitHub Actions `AAIS CI` has passed on remote `main`, including dependency audit, source hygiene, product CI, Playwright install, and E2E. Classic branch protection and repository rulesets were both rejected by GitHub for this private repo with HTTP 403 requiring GitHub Pro or a public repository. Provider-side protected `main`, PR-based CI proof, Vercel Git integration, and direct-deploy permission lockout remain pending. |

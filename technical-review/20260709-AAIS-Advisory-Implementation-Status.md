...
| T-03 Private GitHub repo, protected main, CI | Partially implemented; provider plan action pending | Private repo `https://github.com/HUDongpin/AAIS` exists and commit `c3f5261` is pushed to remote `main`; `.github/workflows/ci.yml`, `.github/workflows/preview-e2e.yml`, Dependabot, repo hygiene checks, dependency-audit guardrails, and the Vercel production build guard in `scripts/guard-vercel-production-deploy.mjs` are present. Classic branch protection and repository rulesets were both rejected by GitHub for this private repo with HTTP 403 requiring GitHub Pro or a public repository. Provider-side protected `main`, green GitHub Actions run, Vercel Git integration, and direct-deploy permission lockout remain pending. |
...
- T-03 protected `main` on GitHub, green provider-side CI on a PR, Vercel Git deploy flow, and provider-side direct-deploy lockout. The private GitHub repo and remote `main` push are complete, but GitHub rejected branch protection/rulesets on the private repo until the account/repo has GitHub Pro or the repo is public.

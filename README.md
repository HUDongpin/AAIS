...
The Aliyun workflow first runs product gates in a GitHub-hosted job with no OIDC permission, then builds the Node 24 standalone image in a separate publish job that exchanges GitHub OIDC for short-lived Alibaba Cloud STS. It temporarily allows only that hosted runner's verified IPv4 `/32` on a dedicated ACR Enterprise public endpoint and closes the endpoint before issuing the exact-digest receipt. A separate GitHub `workflow_run` cleanup verifies or closes that exact run again after completion or cancellation. No second ECS is required and the shared production ECS never builds source. Its deploy wrapper pulls the same digest through the ACR VPC endpoint and binds AAIS to loopback ports 3101/3102. The default secret bootstrap uses one encrypted-disk, root-only local source file and derives the worker view from the same snapshot; paid KMS is optional and requires separate price approval. See [the Aliyun primary runbook](./docs/aliyun-primary-runbook.md) before creating billable resources, migrating data, or changing DNS.
...
- [docs/aliyun-primary-runbook.md](./docs/aliyun-primary-runbook.md): Aliyun primary, private RDS migration, Vercel cold backup without the Static IP add-on, rollback, cost-approval, and evidence gates.

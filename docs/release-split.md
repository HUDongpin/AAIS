...
已实现 [独立无凭据原型](terminal-confirm-prototype.md)，尚待Owner交互验收、未激活生产。
普通操作确认不证明硬件身份、签名密钥保护或服务器权限。
...
后续 [原生确认/临时签名演示](confirmation-check.md) 不包含生产密钥保护；
[ECS 接收端审查](ecs-owner-receiver-review.md) 要求继续封堵，不能接受离线演示结果。

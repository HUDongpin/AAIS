...
- `browser-network-summary.json` must cover exactly the same three synthetic browser contexts and the complete visit/event window. Every HTTP(S) request must pass the exact-origin route guard and have one terminal ledger row; non-local/invalid requests, unguarded or in-flight requests, WebSockets, service workers, observer errors, and retained raw network fields must all be zero. The full-coverage P1 context must causally observe exactly one synthetic download event from the declared “下载到本地” click, while P2 and P3 must observe none. Playwright runs with download-file retention disabled, so no downloaded artifact, filename, path, or content is retained. The manifest-byte SHA-256 binds this audit to the predeclared actions.

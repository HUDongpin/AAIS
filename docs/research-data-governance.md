...
- A visit may contain at most 10,000 distinct research events. The database enforces this before the event/outbox insert and the API returns `AAIS_RESEARCH_EVENT_LIMIT_REACHED` without adding either row. An exact retry of an already recorded `client_event_id` remains idempotent and returns the original event; it does not consume another slot.
...
- The institutional access register records every approved identity-map/raw-text access and retention exception with actor, purpose, scope, timestamp, and outcome. The application writes count-only receipts for successful exports, retention runs, withdrawals, and LRS deletions; denied or failed API attempts enter the redacted security log. Each application security entry is emitted once and contains only the fixed route, operation, HTTP status, fixed error kind, authentication mode, and `secrets: redacted`. It never contains a cookie, bearer credential, CSRF value, `studyRunId`, raw actor id, request or response body, or original error text. No audit surface records raw text.
...
- `browser-network-summary.json` must cover exactly the same three synthetic browser contexts and the complete visit/event window. Every HTTP(S) request must pass the exact-origin route guard and have one terminal ledger row; non-local/invalid requests, unguarded or in-flight requests, WebSockets, service workers, downloads, observer errors, and retained raw network fields must all be zero. The synthetic picker exercises the declared save/download UI outcomes without writing a browser download file. The manifest-byte SHA-256 binds this audit to the predeclared actions.
